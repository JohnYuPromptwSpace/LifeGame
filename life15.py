...
LIFE_NAME = {"Block":8,"Blinker":9,"Glider stopper":10,"Toad":0, "Beacon":1, "Glider":2, "Pulsar":3, "48P22.1":4, "Achim's p11":5, "Flicker":6, "Gosper Glider Gun":7,"Buckaroo":11}
...
# load patterns
def load_data_patterns(file_path):
    with open(file_path, 'r') as file:
        lines = file.readlines()
...
show_selected = True if settings_data[2] == "Show" else False
show_cell_pos = True if settings_data[3] == "Show" else False
...
for PUZZLE_MAP in LAB_NAME:
    LIFE_PATTERNS.append(load_data_patterns(f"data/puzzles/{PUZZLE_MAP}.txt"))

LIFE_PATTERNS.append([[0,0],[1,0],[0,1],[1,1]])
LIFE_PATTERNS.append([[0,0],[1,0],[2,0]])
LIFE_PATTERNS.append([[0,0],[0,1],[1,0],[2,1],[2,2],[2,3],[3,3]])
LIFE_PATTERNS.append([[9, 0],[7, 1],[9, 1],[6, 2],[8, 2],[0, 3],[1, 3],[5, 3],[8, 3],[0, 4],[1, 4],[6, 4],[8, 4],[7, 5],[9, 5],[19, 5],[20, 5],[9, 6],[19, 6],[21, 6],[21, 7],[21, 8],[22, 8]])
...
def draw_loading_screen(progress):
    ...
    title_rect = title.get_rect(center=(WIDTH // 2, HEIGHT // 13*6))
...
def main_game():
    ...
    # def overlay surface
    overlay_surf = pygame.Surface((MAP_WIDTH * GAME_CELL_SIZE, MAP_HEIGHT * GAME_CELL_SIZE), pygame.SRCALPHA)
    rect_surf = pygame.Surface((MAP_WIDTH * GAME_CELL_SIZE, MAP_HEIGHT * GAME_CELL_SIZE), pygame.SRCALPHA)
    ...
    def get_min_max(data):
        # set default value
        min_first = max_first = data[0][0]
        min_second = max_second = data[0][1]
        ...
        return min_first, max_first, min_second, max_second
    
    # rotate pattern
    def rotate_indices_counter_clockwise(indices):
        min_first, max_first, min_second, max_second = get_min_max(indices)
        size = max_first - min_first
        rotated_indices = []
        for i, j in indices:
            rotated_indices.append([size - 1 - j, i])
        
        # Find the minimum x and y values to shift all coordinates
        min_x = min(x for x, y in rotated_indices)
        min_y = min(y for x, y in rotated_indices)
        
        # Adjust all coordinates to make the top-left one [0, 0]
        adjusted_indices = [[x - min_x, y - min_y] for x, y in rotated_indices]
        
        return adjusted_indices
    
    # Function to place a pattern on the grid
    def place_pattern(pattern, center_x, center_y):
        min_first, max_first, min_second, max_second = get_min_max(pattern)
        w = max_first - min_first
        h = max_second - min_second
        for rel_x, rel_y in pattern:
            abs_x = center_x + rel_x - w//2 + camera_x//GAME_CELL_SIZE
            abs_y = center_y + rel_y - h//2 + camera_y//GAME_CELL_SIZE
            if 0 <= abs_x < MAP_WIDTH and 0 <= abs_y < MAP_HEIGHT:
                interesting[abs_y * MAP_WIDTH + abs_x] = LIVE

            pygame.draw.rect(world_surface, LITTLEBRIGHTORANGE, (abs_x * GAME_CELL_SIZE, abs_y * GAME_CELL_SIZE, GAME_CELL_SIZE, GAME_CELL_SIZE))

    # Function to draw the pattern overlay
    def draw_pattern_overlay(screen, pattern, center_x, center_y):
        overlay_surf.fill((0, 0, 0, 0))
        min_first, max_first, min_second, max_second = get_min_max(pattern)
        w = max_first - min_first
        h = max_second - min_second
        for rel_x, rel_y in pattern:
            abs_x = center_x + rel_x - w//2 + camera_x//GAME_CELL_SIZE - camera_x / GAME_CELL_SIZE
            abs_y = center_y + rel_y - h//2 + camera_y//GAME_CELL_SIZE - camera_y / GAME_CELL_SIZE
            if -1 <= abs_x < MAP_WIDTH + 1 and -1 <= abs_y < MAP_HEIGHT+1:
                pygame.draw.rect(overlay_surf, (0, 255, 0, 128), (abs_x * GAME_CELL_SIZE, abs_y * GAME_CELL_SIZE, GAME_CELL_SIZE, GAME_CELL_SIZE))
    ...
    def draw_transparent_rect(screen, start_x, start_y, end_x, end_y, color):
        ...
        rect_w = abs(start_x - end_x) + 1
        rect_h = abs(start_y - end_y) + 1
        pygame.draw.rect(rect_surf, color, (rect_x, rect_y, rect_w, rect_h))
        screen.blit(rect_surf, (0, 0))
    
    def remove_cells(start_x, start_y, end_x, end_y):
        start_x, end_x = map(lambda x : (x + camera_x)//GAME_CELL_SIZE,[start_x, end_x])
        start_y, end_y = map(lambda x : (x + camera_y)//GAME_CELL_SIZE,[start_y, end_y])
        
        pygame.draw.rect(world_surface, COBALT, (min(start_x, end_x) * GAME_CELL_SIZE, min(start_y, end_y) * GAME_CELL_SIZE, (abs(start_x - end_x)+1) * GAME_CELL_SIZE, (abs(start_y - end_y)+1) * GAME_CELL_SIZE))
        for y in range(min(start_y, end_y), max(start_y, end_y)+1):
            for x in range(min(start_x, end_x), max(start_x, end_x)+1):
                if 0 <= x < MAP_WIDTH and 0 <= y < MAP_HEIGHT:
                    try:
                        del interesting[x + y * MAP_WIDTH]
                    except:
                        pass
    ...
    # Scroll bar init  
    life_create_buttons = lifx.ScrollableItemList(screen,12,100,150,3,SCREEN_WIDTH,SCREEN_HEIGHT-119,15,fontP,LIFE_NAME.keys())
    ...
    while running:
        ...
        for event in pygame.event.get():
            if dialogs:
                ...
                if currDialog.title == "Save and Exit Life?":
                    ...
                    if currDialog.handle_event(event) == True:
                        remove_cells(rect_start_x, rect_start_y, rect_end_x, rect_end_y)
                        del currDialog
                        dialogs.clear()
                    elif currDialog.handle_event(event) == False:
                        del currDialog
                ...
                other = False
                if scroll_bar_show and not minimap_fullscreen and manage_mode == 1:
                    ...
                    if a != 1:
                        ...
                        if a != 2:
                            try:
                                if player_state[0][LIFE_NAME[a]] == 1:
                                    selected = a
                            except:
                                if a == "Buckaroo":
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_f:  # Toggle fullscreen minimap with 'F' key
                        ...
                        if manage_mode == 1 and not minimap_fullscreen and selected:
                            LIFE_PATTERNS[LIFE_NAME[selected]] = rotate_indices_counter_clockwise(LIFE_PATTERNS[LIFE_NAME[selected]])
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if buttons[0][0].collidepoint(event.pos) and not minimap_fullscreen:
                        other = True
                        main_button_expanded = not main_button_expanded
                    if main_button_expanded:
                        if buttons[2][0].collidepoint(event.pos) and not minimap_fullscreen:
                            other = True
                            if buttons[2][3] == "Spectate":
                                ...
                                pause = True
                                mode = 1
                        if mode == 0 and buttons[1][0].collidepoint(event.pos) and not minimap_fullscreen:
                            other = True
                            if buttons[1][3] == "Stop":
                                buttons[1][1] = button_font.render('Play', True, BRIGHTORANGE)
                                buttons[1][3] = "Play"
                                ...
                                pause = False
                    if mode == 1:
                        if manage_buttons[0][0].collidepoint(event.pos) and not minimap_fullscreen:
                            other = True
                            if manage_mode == 1:
                                manage_mode = 0
                                selected = None
                                ...
                                manage_mode = 1
                                scroll_bar_show = False
                        elif manage_buttons[1][0].collidepoint(event.pos) and not minimap_fullscreen:
                            other = True
                            if manage_mode == 2:
                                manage_mode = 0
                            else:
                                manage_mode = 2
                                selected = None
                    if scroll_bar_toggle.collidepoint(event.pos) and not minimap_fullscreen and manage_mode == 1:
                        other = True
                        scroll_bar_show = not scroll_bar_show
                    if manage_mode == 1 and selected != None and not other and not minimap_fullscreen and event.button == 1:
                        place_pattern(LIFE_PATTERNS[LIFE_NAME[selected]], mouse_pos[0]//GAME_CELL_SIZE, mouse_pos[1]//GAME_CELL_SIZE)
                    if manage_mode == 2 and not other and not minimap_fullscreen:
                        if event.button == 1:  # Right mouse button
                            drawing_rect = True
                            rect_start_x, rect_start_y = mouse_pos[0], mouse_pos[1]
                if mode == 0:
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        if dragging:
                            ...
                            camera_x = max(0, min(camera_x, WORLD_WIDTH - SCREEN_WIDTH))
                            camera_y = max(0, min(camera_y, WORLD_HEIGHT - SCREEN_HEIGHT))
                if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and manage_mode == 2 and drawing_rect:
                    drawing_rect = False
                    rect_end_x, rect_end_y = mouse_pos
                    killLifeDia = lifx.DialogBox(screen, "Kill Lifes?", "Press ENTER to Continue\nPress ESCAPE to Discard", width=WIDTH//5, height=HEIGHT//5)
                    dialogs.append(killLifeDia)
        ...
        if drawing_rect and not minimap_fullscreen and not dialogs:
            rect_end_x = mouse_pos[0]
            rect_end_y = mouse_pos[1]
            draw_transparent_rect(screen, rect_start_x, rect_start_y, rect_end_x, rect_end_y, (255, 0, 0, 128))
        
        # create life
        if manage_mode == 1 and not dialogs and selected != None and not minimap_fullscreen:
            draw_pattern_overlay(screen, LIFE_PATTERNS[LIFE_NAME[selected]], mouse_pos[0]//GAME_CELL_SIZE, mouse_pos[1]//GAME_CELL_SIZE)
        
        # blit log / scores
        if not minimap_fullscreen:
        ...
        if scroll_bar_show:
            # display scroll bar
            if manage_mode == 1 and minimap_fullscreen == False and not dialogs:
                life_create_buttons.update(COBALT,BRIGHTCOBALT,BRIGHTORANGE,BRIGHTORANGE,DARKORANGE,DARKCOBALT,life_create_expand_width-165,CELL_SIZE,selected,player_state,LIFE_NAME)
        ...
        if display_FPS and not minimap_fullscreen:
            lifx.blitFPS(screen, fontP, FPS_clock, BRIGHTORANGE)

        adji = 2
        if show_selected and not minimap_fullscreen:
            life_log = fontP.render(str(selected), True, BRIGHTORANGE)
            screen.blit(life_log, (WIDTH - MINIMAP_WIDTH, MINIMAP_HEIGHT+10+log_text_height*adji))
            adji+=1
        
        if show_cell_pos and not minimap_fullscreen:
            life_log = fontP.render(str(mouse_pos[0]//GAME_CELL_SIZE+camera_x//GAME_CELL_SIZE) + " " + str(mouse_pos[1]//GAME_CELL_SIZE+camera_y//GAME_CELL_SIZE), True, BRIGHTORANGE)
            screen.blit(life_log, (WIDTH - MINIMAP_WIDTH, MINIMAP_HEIGHT+10+log_text_height*adji))
            adji+=1
...
def settings():
    global current_screen, selected_lab, display_FPS,FPS, display_FPS, show_selected, show_cell_pos
    running = True
    ...
    selected_Show_dropdown = lifx.Dropdown(HEIGHT // 8, HEIGHT // 13*5.5 + int(CELL_SIZE * 4)*1.5, 200, 40, ["Hide","Show"], fontP, BRIGHTCOBALT, BRIGHTORANGE,BRIGHTCOBALT, BRIGHTERCOBALT, default_text=f"{settings_data[2]}")
    cell_pos_Show_dropdown = lifx.Dropdown(HEIGHT // 8, HEIGHT // 13*7 + int(CELL_SIZE * 4)*1.5, 200, 40, ["Hide","Show"], fontP, BRIGHTCOBALT, BRIGHTORANGE,BRIGHTCOBALT, BRIGHTERCOBALT, default_text=f"{settings_data[3]}")

    active_dropdown = False
    not_active_dropdown = [FPS_dropdown,FPS_Show_dropdown,selected_Show_dropdown,cell_pos_Show_dropdown]
    ...
    while running:        
        ...
        for event in pygame.event.get():
            ...
            # selected_Show_dropdown
            if active_dropdown == selected_Show_dropdown or not active_dropdown:
                newShowFPS = selected_Show_dropdown.handle_event(event)
                if newShowFPS != None:
                    if newShowFPS == "Show":
                        show_selected = True
                    else:
                        show_selected = False
                    settings_data[2] = newShowFPS
            
            # cell_pos_Show_dropdown
            if active_dropdown == cell_pos_Show_dropdown or not active_dropdown:
                newShowFPS = cell_pos_Show_dropdown.handle_event(event)
                if newShowFPS != None:
                    if newShowFPS == "Show":
                        show_cell_pos = True
                    else:
                        show_cell_pos = False
                    settings_data[3] = newShowFPS
        ...
        selected_title = fontP.render("Show Selected Life", True, BRIGHTORANGE)
        screen.blit(selected_title, (HEIGHT // 8, HEIGHT // 13*5.5))

        cell_pos_title = fontP.render("Show Life Pos", True, BRIGHTORANGE)
        screen.blit(cell_pos_title, (HEIGHT // 8, HEIGHT // 13*7))
