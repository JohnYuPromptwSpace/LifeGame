"""
v0.1.00
Optimized by changing every list to dictionary
Added settings page
Added puase button and manage button
"""
...
def main_game():
    ...
    @njit
    def neighbors(MAP_WIDTH, MAP_HEIGHT,r, c):
        neighbor_indices = [
    ...
    button_font = pygame.font.Font(None, CELL_SIZE*8)

    # Define the button text
    buttons = []
    
    button_pause_rect = pygame.Rect(270, 190, 150, 50)
    ...
    buttons.append([button_pause_rect,button_pause_text,button_pause_text_rect, "Stop"])
    
    button_mode_rect = pygame.Rect(500, 190, 200, 50)
    ...
    buttons.append([button_mode_rect,button_mode_text,button_mode_text_rect, "Manage"])
    ...
    while running:
        ...
        for event in pygame.event.get():
            if dialogs:
                ...
                if buttons[1][0].collidepoint(event.pos):
                    if buttons[1][3] == "Spectate":
                        buttons[1][1] = button_font.render('Manage', True, BRIGHTORANGE)
                        buttons[1][3] = "Manage"
                        buttons[0][1] = button_font.render('Stop', True, BRIGHTORANGE)
                        buttons[0][3] = "Stop"
                        pause = False
                        mode = 0
                    else:
                        buttons[1][1] = button_font.render('Spectate', True, BRIGHTORANGE)
                        buttons[1][3] = "Spectate"
                        buttons[0][1] = button_font.render('Play', True, BRIGHTORANGE)
                        buttons[0][3] = "Play"
                        pause = True
                        mode = 1
                if mode == 0:
        ...
        if not pause:
            ...
            if SCREEN_UPDATE_TICK_CNT >= SCREEN_UPDATE_TICK * FPS_clock.get_fps() / 30:
                ...
                for cell in cells:
                    ...
                    for neighbor in neighbors(MAP_WIDTH, MAP_HEIGHT, r, c):
        ...
        current_button_color = BRIGHTERCOBALT
        if mode == 0:
            if buttons[0][0].collidepoint(mouse_pos):
                current_button_color = BRIGHTCOBALT
            pygame.draw.rect(screen, current_button_color, buttons[0][0])
        # Draw the text on the button
        screen.blit(buttons[0][1], buttons[0][2])
        
        current_button_color = BRIGHTERCOBALT
        if buttons[1][0].collidepoint(mouse_pos):
            current_button_color = BRIGHTCOBALT
        pygame.draw.rect(screen, current_button_color, buttons[1][0])
        # Draw the text on the button
        screen.blit(buttons[1][1], buttons[1][2])
...
def settings():
    global current_screen, selected_lab, display_FPS,FPS
    running = True
    
    settings_data = load_data_settings("data/player_data/settings.txt")

    FPS_dropdown = lifx.Dropdown(HEIGHT // 13, HEIGHT // 13*2.5 + int(CELL_SIZE * 4)*1.5, 200, 40, ["30", "45", "60", "90","120"], fontP, BRIGHTCOBALT, BRIGHTORANGE,BRIGHTCOBALT, BRIGHTERCOBALT, default_text=f"{settings_data[0]}")
    ...
    while running:        
        ...
        for event in pygame.event.get():
            ...
            FPS = int(newFPS) if newFPS != None else FPS
            settings_data[0] = FPS
        ...
        lab_title = fontP.render("Select FPS", True, BRIGHTORANGE)
        screen.blit(lab_title, (HEIGHT // 13, HEIGHT // 13*2.5))
